...
class OAuthUtil { 
    ...
    public static function build_http_query_multi($params) { 
        ...
        if( in_array($parameter,array("pic","image")))
        {
			...
			$tmp_arr =  explode( '?' , basename( $url ) );			
			$filename = reset($tmp_arr);
...
class OAuthLite { 
    ...
    //public $host = "http://api.t.sina.com.cn/oauth"; 
    public $host;
...
{	
	...
	{
		$this->oauth = new OAuthLite( $akey , $skey , $accecss_token , $accecss_token_secret );
		$this->oauth->init($this->host.$this->host_dir);
	}
	
	function upload($url,$param,$multi = false)
	{
		return $this->oauth->post( $url , $param ,$multi );
	}
...
class Sina extends Client
{
	...
	function __construct($akey , $skey , $accecss_token , $accecss_token_secret )
	{
		...
		$version = '1.0a';
		OAuthRequest::set_version($version);
	}
	
	function send($text,$pic_path = '')
	{
		$param = array();
		$param['status'] = $text;
		$multi = false;
		...
		return $this->upload($url,$param,$multi);
	}
}
...
class Qq extends Client
{
	...
	function send($text,$pic_path = '')
	{
		...
		$multi = false;
		$url = $this->host.'/api/t/add?f=1';
		
		if(isset($pic_path{0}))
		{
			$multi = true;
			$param['pic'] = $pic_path;
			$url = $this->host.'/api/t/add_pic?f=1';
